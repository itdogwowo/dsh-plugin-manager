...
> ⚠️ **移動本地 checkout 需要 `git`**：這台機器上沒有（PATH 沒有、官方安裝路徑沒有、登錄檔也沒有）。
> 所以那個按鈕會**明說**缺什麼，並給出替代做法（把來源改成 `github:<owner>/<repo>#<ref>`）。
>
> **而且它會給一條出路（跨平台）**：`plan` 會照宿主讀到的平台附上安裝方式 ——
> Windows 給 `winget install --id Git.Git -e --source winget`、macOS 給 `xcode-select --install`、
> Linux 讀 `/etc/os-release` 後給對應的套件管理器指令（apt／dnf／zypper／pacman／apk／xbps，
> 也讀 `ID_LIKE` 所以 Mint 這類衍生版不會落空）。面板把它顯示成一段可複製的指令
> ＋一顆連到官方安裝說明的按鈕。**面板不會替你安裝**——它是一個插件，跑套件管理器需要
> 使用者同意流程，那不是它能做的事；指令要你自己貼進終端機、看過再跑。
> 讀不到 `/etc/os-release` 的發行版**不會**拿到一條猜的指令，只會拿到官方連結。
...
| 從面板**安裝**新插件 | ✅ | spec 守門 → 計畫預覽（顯示會跑的指令）→ 走完整管道；拒絕時附上官方 CLI 的替代指令 |
...
**裝完要重啟 `dsh web`** 才會載入（面板會這樣說）。

> ⚠️ **spec 會被檢查。** 開頭是 `-`、含控制字元、含 shell 特殊字元（`$;&|<>()` 等）、
> 非 ASCII、或超過 1024 字元的 spec 會在**送出之前**被拒，並附上理由。
> 這是刻意的從嚴：面板是方便，不是唯一入口——被擋掉的 spec 你仍然可以用官方 CLI 裝，
> 拒絕訊息裡就附著那行指令。
>
> ⚠️ 本機路徑請用**正斜線**（`link:../dir`、`link:C:/Users/<你的帳號>/dir`）。反斜線會被拒。

### 裝一顆新插件（官方 CLI）

面板讀不到 profile、或你想跳過管道時，用官方指令：
...
⚠️ 走 CLI **不經過管道**——沒有裝前驗證、沒有快照、不會自動回滾。
...
- **安裝欄位** — 輸入 spec → 先看計畫（會跑的指令、`dsh` 在不在、是新增還是改寫）→ 才出現安裝鈕
...
生態裡其他管理器把輸入框和「安裝」鈕放在一起，那顆鈕就變成一次信仰之躍。
本插件的賣點是**答案在動手之前就到**，所以輸入框不能例外：
打字不產生任何東西，按「檢查計畫」才產生計畫，而**只有可執行的計畫才會給出安裝鈕**。

計畫會明說三種不同的處境，因為它們的後果不同：

| 顯示 | 意思 |
|---|---|
| 「這顆目前不在 profile 裡」 | 新增 |
| 「這會改寫已記錄的 spec：`<舊的>`」 | 取代——舊值會被寫掉，所以先講 |
| 「profile 已經記著同一個 spec」 | 重跑只會重新解析來源（pinned 版本是 no-op；URL／tag 是重新抓取） |

還有一種情況**不會**被說成上面任何一種：spec 看不出會裝成什麼名字時（例如 `^1.2.0`、
`link:../dir`），面板會說「看不出來」，而不是猜「這是新的」。
猜錯的方向剛好是最危險的那一邊。
...
在卡片右邊、**開關的正下方**按「檢查更新」（面板展開後這顆鈕就收起來，面板裡有「載入版本」可以重讀）：
